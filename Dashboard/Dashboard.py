...
def create_monthly_sharing(df):
    ...
    monthlyshare_df = df_2011.resample(rule='ME', on='dteday').agg({
        "casual": "sum",
        "registered": "sum",
        "cnt": "sum",
        "windspeed": "mean",
...
ax.bar(x - width / 2, byweathersit_df['casual'], width, color=colors1)
ax.bar(x + width / 2, byweathersit_df['registered'], width, color=colors2)
...
ax.plot(x, monthlysum_df['windspeed'], marker='o',color='#0000FF')  
ax.plot(x, monthlysum_df['weathersit'], marker='o',color='#87CEEB') 
